      ...
      const filteredClients = clients.filter((client) => {
        ...
        return Object.entries(filter).every(([field, value]) => {
          ...
          if (typeof value === "string") {
            if (value.startsWith("%") && value.endsWith("%")) return client[field].includes(value.substr(1, value.length - 2));
            if (value.startsWith("%")) return client[field].endsWith(value.substr(1));
            if (value.endsWith("%")) return client[field].startsWith(value.substr(0, value.length - 1));
